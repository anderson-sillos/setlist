...
91. A tarefa 5.5 implementou a administração real de integrantes. `BandScreen` mantém os grupos de Proprietários, Editores e Integrantes em ordem alfabética, marca a própria pessoa e mostra os controles somente para Owners. Ações de promover para Proprietário e remover integrante usam um diálogo de confirmação que identifica a pessoa; as mutações chamam o Supabase e traduzem erros de permissão ou do último Owner. Bandas demonstrativas continuam exibindo um aviso sem alterar dados. Foram adicionados testes de mutações, confirmações, ordenação e ausência de controles para Editor e Member. Formatação, lint, TypeScript e 50 suítes (248 testes) passaram; a PR #13 continua aberta.
