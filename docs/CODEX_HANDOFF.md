...
69. O primeiro login na prévia do GitHub Pages revelou que o `redirectTo` web ainda era montado pelo `Linking.createURL()` como `setlist://auth/callback`. `getDevelopmentUrl` agora usa a origem do navegador e identifica o base path pelo bundle `/_expo/`, preservando `/setlist/app/auth/callback` no GitHub Pages e mantendo o esquema nativo em Android/iOS. A configuração do projeto Supabase de desenvolvimento foi atualizada pela CLI para aceitar `https://anderson-sillos.github.io/setlist/app/auth/callback**`, incluindo o parâmetro `sb_flow_id`; o retorno nativo e os demais padrões foram preservados. Testes cobrem o callback web hospedado.
