...
41. A tarefa 4.1 começou na branch `feat/supabase-environments`. O cliente tipado em `src/data/supabase/client.ts` usa somente URL e chave publicável, os perfis EAS selecionam `development` e `production`, e os modelos `.env.development.example` e `.env.production.example` separam os projetos. O comando `npm run supabase:check -- development|production` testa o endpoint público sem expor credenciais; a tarefa permanece pendente até os dois projetos hospedados e suas chaves reais serem configurados e verificados.
