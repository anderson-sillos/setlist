...
51. Uma nova tentativa ainda exibiu a mensagem genérica de falha no botão do Google. O digest móvel passou a usar diretamente `expo-crypto`, sem reaproveitar uma implementação parcial de `crypto.subtle`, e o serviço agora normaliza e registra no Metro qualquer exceção inesperada como `oauth_unexpected_error`. A próxima validação deve conferir o log dessa exceção e se o provedor Google está habilitado no mesmo projeto Supabase selecionado por `EXPO_PUBLIC_APP_ENV`.
